...
const confirmClose = () => {
    confirm.require({
        message: i18n.t('forms.closeFormConfirm') + '\n' + i18n.t('general.irreversible'),
        header: i18n.t('forms.closeForm'),
        icon: 'pi pi-lock',
        rejectLabel: i18n.t('general.cancel'),
        rejectProps: {
          label: i18n.t('general.cancel'),
        ...
        acceptProps: {
          label: i18n.t('forms.close'),
          severity: 'danger'
        },
        accept: () => {
          try {
            const response = closeFormReq(form.value.id);
          } catch (e) {
            console.log(e?.response?.data)
          }
        },
      ...
      >
        <div class="flex gap-4 justify-between">
          <div>
            ...
            <p v-if="form.description" class="text-xl mt-2!">{{ form.description }}</p>
            <p v-if="form.closed" class="mt-2! text-red-700"><i>{{ $t('forms.closedMessage') }}</i></p>
          </div>
          <div class="flex gap-4">
            <div class="flex gap-1">
              <Button
                ...
                :title="form.closed ? $t('forms.closedMessage') : ''"
            ...
            <div class="flex gap-1">
              ...
              >
                ...
                <span class="hidden md:inline">{{ $t("forms.close") }}</span>
              </Button>
              <Button
                @click="confirmDelete"
                severity="danger"
